...
class DataAugmentForObjectDetection():
    def __init__(self, change_light_rate=0.5,
        ...
        # 是否使用某种增强方式
        self.is_addNoise = is_addNoise  # 加噪声
        self.is_changeLight = is_changeLight  # 改变亮度
        self.is_random_point = is_random_point  # 加随机点
        self.is_filp_pic_bboxes = is_filp_pic_bboxes  # 翻转
        self.is_shift_pic_bboxes = is_shift_pic_bboxes  # 平移
        self.is_rotate = is_rotate  # 旋转
    ...
    # 图像旋转
    def _rotateImage(self, img, json_info):
        ...
        for shape in shapes:
            for p in shape['points']:
                x, y = p
                # Rotate point (x, y) around center (cols / 2, rows / 2)
                rotated_x = (x - cols / 2) * math.cos(math.radians(angle)) - (y - rows / 2) * math.sin(
                    math.radians(angle)) + cols / 2
                rotated_y = (x - cols / 2) * math.sin(math.radians(angle)) + (y - rows / 2) * math.cos(
                    math.radians(angle)) + rows / 2
                p[0], p[1] = rotated_x, rotated_y

        return rotated_img, json_info
    ...
    def dataAugment(self, img, dic_info):
        ...
        while change_num < 1:  # 默认至少有一种数据增强生效
            ...
            if self.is_rotate:
                if random.random() < self.rotate_rate:  # 旋转角度
                    change_num += 1
                    img, bboxes = self._rotateImage(img, dic_info)
...
if __name__ == '__main__':
    ...
    original_dir = "../data/test_aug"  # 原始数据目录，包含json文件
    result_dir = "../data/test_ratate"  # 增强后保存的目录，包含json文件
    # 获取相关参数
    parser = argparse.ArgumentParser()
    parser.add_argument('--source_img_json_path', type=str, default=original_dir)
    parser.add_argument('--save_img_json_path', type=str, default=result_dir)
